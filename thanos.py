...
async def send_vid(bot: Client, m: Message, cc, filename, thumb, name, prog, channel_id, watermark="Thanos", topic_thread_id: int = None):
    try:
        ...
        # ✅ Topic kwargs (only if topic_thread_id provided)
        topic_kw = {"message_thread_id": topic_thread_id} if topic_thread_id else {}
        ...
        if file_size_mb < 2000:
            ...
            try:
                sent_message = await bot.send_video(
                    ...
                    thumb=thumbnail,
                    duration=dur,
                    message_thread_id=topic_thread_id,   # ✅ TOPIC UPLOAD
            ...
            if os.path.exists(filename):
                os.remove(filename)
            try: await reply.delete(True)
            except: pass
            try: await reply1.delete(True)
            except: pass
            ...
            try:
                ...
                for idx, part in enumerate(parts):
                    ...
                    try: await upload_msg.delete(True)
                    except: pass
            ...
            if notify_split:
                try: await notify_split.delete(True)
                except: pass
            if os.path.exists(filename):
                os.remove(filename)
